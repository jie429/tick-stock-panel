...
- **自选** Watchlist — 自选股池,多分组管理(M:N),表格/卡片双视图,换手/量比/RSI 等实时指标,按档位分流实时刷新;点击任意标的打开**个股详情弹窗**(日K + 点某日展开的分时图;分时图右侧并排盘口面板:五档价/量 + 内盘外盘成交方向,时轴最前一格为 09:25 集合竞价柱 + 竞价成交比开关)
...
- **异动监控** Abnormal Moves — 按交易时间线三 tab:
  - **竞价异动** — 同花顺盘前风向标(含当日/次日真实收益对照、追高风险标记)+ 全市场竞价扫描(09:25 竞价终态一次扫全市场: 竞价额/竞价量/竞价量比/封单额/内外盘, 按日落盘积累; 同一份快照供个股分时图 09:25 竞价柱与竞价成交比)
...
flowchart TB
    subgraph DATA["数据源层 · 插件化"]
        direction LR
        D1["TickFlow SDK"] ~~~ D2["fuyao<br/>同花顺 REST"] ~~~ D3["麦蕊智数<br/>沪深 REST"] ~~~ D4["stock-sdk"] ~~~ D5["YAML 自定义源"] ~~~ D6["+ 更多插件…"]
    end
