...
- **个股详情弹窗盘口卡片**(自选 / 连板梯队 / 异动 / 概念行业 / 复盘等入口共用, **只在分时视图显示** —— 盘口与成交方向都是当日实时/当日累计数据, 与历史日K不是同一时间尺度):五档买卖报价(价 + 量, 量单位为「手」, 与封单量同口径)+ 当日成交方向(内盘 / 外盘与主动买占比),并标注盘口快照时间;价与量取自同一次快照,缺档显示 `—` 不补 0,成交方向数据源未提供时只隐藏该段。走**五档盘口**能力路由,未配置数据源时给出配置入口;交易时段 15s 轮询,休盘不空转
