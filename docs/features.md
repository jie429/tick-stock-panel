...
| Tab | 口径 |
| :--- | :--- |
| 竞价异动(盘前) | ① fuyao 同花顺盘前风向标:当日竞价强势榜(5-6 只/日,含竞价涨幅与概念标签),附带**当日 / 次日真实收益对照**与**追高风险标记**(高开 ≥5% 追高当日均值显著为负,来自 60 日回测);② **全市场竞价扫描**(实时行情源可选协议,eltdx 支持):9:25 竞价终态一次扫全市场(实测 5500+ 只约 4 秒),筛「高开 ≥5% 且竞价量比 ≥10」,同时给出竞价额、封单额与内外盘(成交方向);竞价量比 = 今日竞价量 ÷ 上一可得快照日竞价量,按日落盘自启用之日起积累 |
...
盘前风向标来自 fuyao 数据源(历史按日缓存,当日不缓存),未配置 fuyao key 时该卡片降级为引导提示;**全市场竞价扫描**来自「实时行情」数据源,未实现该可选协议时同样降级提示。竞价量比需至少两份按日落盘快照(首日按竞价额排序),快照落在 `data/auction_scan/date=YYYY-MM-DD.parquet`。
...
- **个股详情弹窗盘口面板**(自选 / 连板梯队 / 异动 / 概念行业 / 复盘等入口共用):贴在**日K右侧那张分时图**旁边竖排并排(点日K某日展开分时图时随之出现, 与分时图等高)—— 盘口与成交方向都是当日实时/当日累计数据, 属于当日视角, 所以跟当日分时图走, 不跟历史日K。内容为五档买卖报价(卖5→买5 逐档, 价 + 量, 量单位为「手」, 与封单量同口径)+ 当日成交方向(内盘 / 外盘与主动买占比条),并标注盘口快照时间;价与量取自同一次快照,缺档显示 `—` 不补 0,成交方向数据源未提供时只隐藏该段。走**五档盘口**能力路由,未配置数据源时给出配置入口;交易时段 15s 轮询,休盘不空转
- **分时图集合竞价柱 + 竞价成交比**(同一张分时图):时轴最前一格是 `09:25` 集合竞价柱,由当日竞价终态快照合成——竞价按单一价格撮合,故开高低收同为今开,量/额取快照原值(手 / 元);右上「竞价 3.52×」按钮既是**显示开关**又直接给出**竞价成交比**(竞价量比),悬停给出开盘涨幅 / 竞价量额 / 额比 / 占昨日全天成交额,悬停竞价柱时信息条标为「集合竞价」(不与连续交易的分钟柱混读)。源分钟 K 把集合竞价并入当日首根分钟柱(实测全天量额 ≡ Σ分钟柱),因此竞价单独成柱时从首根扣回这一份,全天量额合计与均价线口径不变。读数走**全市场竞价扫描**同一份按日落盘快照(`data/auction_scan/`):历史日只读归档、当日按需扫描(进程内 60s 缓存),无基线时量比显 `—` 不伪造 0,数据源未实现该协议 / 该日无快照 / 该标的无竞价成交时按钮禁用并说明原因(读取中不闪空态;竞价未结束 `not_ready` 时 60s 复查一次)
...
- **板块归属来源二选一**:概念/行业页的归属表出厂只建配置、不联网拉数据,在空态里手动获取 —— ① **同花顺在线接口**(`ext_gn_ths` / `ext_hy_ths`,秒级);② **通达信概念板块**(`ext_gn_tdx`,走 eltdx 可选协议,本机 TCP 逐板块取成分,实测 269 个板块约 40 秒,需已启用 eltdx 数据源)。两者落地的字段与同花顺概念一致,换源后页面 / RPS / 信号 / 因子无需重配
