...
封单额时再乘 `100` 换算为股。同档位的价(`bid_prices` / `ask_prices`, 元)与量必须
取自同一次盘口快照, 补不齐的档位保留 `None`, 不得用 `0` 或昨收凑数。成交方向
(内盘 `inside_volume` / 外盘 `outside_volume`, 手)与全市场竞价快照同字段口径, 单侧
缺失保留 `None`。provider 必须在数据边界完成单位转换。
...
- 边界注记：分时监控由分钟能力兜底（`intraday_monitor_support`），不单设分时能力；`full_minute`（全量分钟）和 `depth5` 数据集均已开放内置 Provider 插件声明。`depth5` 独立路由且失败时不跨源回退；YAML HTTP 自定义源不开放该契约。
