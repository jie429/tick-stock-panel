...
ELTDX_PLUGIN = str(ROOT / "backend" / "app" / "plugins" / "eltdx")
...
# eltdx 是通过 plugin.yaml 字符串动态导入的，PyInstaller 无法沿普通 import
# 链发现。桌面版把其 eltdx TCP 客户端、动态模块和清单一并固化，避免运行时尝试
# 对 frozen exe 执行 pip/uv 安装。
eltdx_d, eltdx_b, eltdx_h = collect_all("eltdx")
datas += eltdx_d
binaries += eltdx_b
hiddenimports += eltdx_h
hiddenimports += collect_submodules("app.plugins.eltdx")
...
# eltdx Provider 的 loader 依赖磁盘 plugin.yaml 扫描; 保留完整目录也让 frozen 状态
# 诊断仍能读取 requirements.txt 和插件说明。
datas += [(ELTDX_PLUGIN, "app/plugins/eltdx")]
