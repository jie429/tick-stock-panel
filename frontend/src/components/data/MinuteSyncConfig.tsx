...
export function MinuteSyncConfig({ hasCap, onJobStart }: { hasCap: boolean; onJobStart?: (jobId: string) => void }) {
  ...
  // 能力尚未加载或请求失败时也必须收口；不能先按 TickFlow 默认放开全市场请求。
  const hasUniverseSync = hasMinuteCap && prefs.data?.minute_universe_sync_supported === true
  ...
  const handleFetch = async (mode: '40d' | '1y') => {
    ...
    try {
      ...
      toast(`分钟K同步未能开始：${message}`, 'error')
    } finally {
      setFetchingMode('')
    }
  }

  const universeSyncHint = !hasMinuteCap
    ? '当前数据源没有分钟K批量能力。'
    : prefs.isPending
      ? '正在确认当前分钟数据源是否支持全市场同步…'
      : prefs.isError
        ? '无法确认当前分钟数据源的全市场同步能力，请刷新后重试。'
        : '当前分钟源仅支持单标的或分组分钟K；请在个股分时图中获取，不支持全市场同步。'
    ...
    <div className="px-4 pb-4 pt-3 border-t border-accent/20 space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <button
            ...
            disabled={!hasUniverseSync}
            className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors duration-200 shrink-0 ${
              effectiveEnabled ? 'bg-accent shadow-[0_0_6px_rgba(61,214,140,0.3)]' : 'bg-elevated'
            } ${!hasUniverseSync ? 'opacity-40 cursor-not-allowed' : 'cursor-pointer'}`}
          >
            <span
          ...
          <span className="text-xs text-foreground font-medium">
            自动同步{effectiveEnabled ? '已开启' : enabled ? '已暂停' : '已关闭'}
          </span>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex items-center">
            <button
              ...
              disabled={!hasUniverseSync || !effectiveEnabled || localDays <= 1}
            ...
            >−</button>
            <div className={`h-6 w-8 flex items-center justify-center border-y border-border text-[11px] font-mono tabular-nums ${effectiveEnabled ? 'text-foreground bg-base' : 'text-muted bg-elevated/50'}`}>
              {localDays}
            </div>
            <button
              onClick={() => setDays(localDays + 1)}
              disabled={!hasUniverseSync || !effectiveEnabled || localDays >= 30}
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center">
            <button
              ...
              disabled={!hasUniverseSync || localSegment <= 5}
              className="h-6 w-6 flex items-center justify-center rounded-l-btn bg-elevated border border-border text-secondary hover:bg-border/50 disabled:opacity-30 transition-colors text-xs"
            >−</button>
            ...
            <button
              onClick={() => setSegment(localSegment + 5)}
              disabled={!hasUniverseSync || localSegment >= 30}
              className="h-6 w-6 flex items-center justify-center rounded-r-btn bg-elevated border border-border text-secondary hover:bg-border/50 disabled:opacity-30 transition-colors text-xs"
            >+</button>
          </div>
      ...
      <div className="pt-3 border-t border-border space-y-2">
        ...
        <button
          ...
          disabled={!hasUniverseSync || fetchingMode !== ''}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative rounded-card border border-border bg-surface shadow-2xl mx-4 px-6 py-5 max-w-sm w-full space-y-4">
            ...
            <div className="text-[11px] text-muted text-center leading-relaxed">
              此操作仅删除分钟K（股票与 ETF）数据, <span className="text-foreground/80">不影响</span>日K、复权因子、指标等其他数据。清空后可重新获取。
            </div>
